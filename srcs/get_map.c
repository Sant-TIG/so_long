#include "../incs/so_long.h"

static ssize_t	ft_read(int fd, char *c, int buffer)
{
	//printf("READ\n");
	ssize_t	bytes;

	bytes = read(fd, c, buffer);
	if (bytes == -1)
		ft_error_exit(READ_ERROR);
	if (bytes == 0)
		return (0);
	return (1);
}

static void	ft_read_map(t_map *map_utils)
{
	//printf("READ_MAP\n");
	size_t	len;
	int		i;

	len = 0;
	i = -1;
	map_utils->str = NULL;
	while (ft_read(map_utils->fd, &map_utils->c, 1) != 0 && ++len)
		ft_insert_end_char(map_utils->c, &map_utils->str);
	map_utils->tmp_map = (char *)malloc(sizeof(char) * len + 1);
	if (!map_utils->tmp_map)
		ft_error_exit(MALLOC_ERROR);
	map_utils->tmp = map_utils->str;
	*(map_utils->tmp_map + len) = '\0';
	while (map_utils->tmp)
	{
		*(map_utils->tmp_map + ++i) = map_utils->tmp->c;
		map_utils->tmp = map_utils->tmp->next;
	}
	*(map_utils->tmp_map + ++i) = '\0';
	ft_free_list(map_utils->str);
	//ft_treat_empty_map(&map_utils);
}

static void	ft_check_extension(const char *file, const char *extension)
{
	//printf("CHECK EXTENSION\n");
	size_t	len;

	if (!file || !extension)
		ft_error_exit(1);
	if (*extension != '.')
		ft_error_exit(INVALID_EXTENSION);
	len = ft_strlen(file) - ft_strlen(extension);
	while(*(file + len) && *extension)
	{
		if (*(file + len) != *extension)
			ft_error_exit(INVALID_EXTENSION);
		len++;
		extension++;
	}
}

void	ft_get_map(char *map, t_game *game)
{
	//printf("GET MAP\n");
	ft_check_extension(map, ".ber");
	game->map_utils->fd = open(map, O_RDONLY);
	if (game->map_utils->fd == -1)
		ft_error_exit(INVALID_FILE);
	ft_read_map(game->map_utils);
	game->map = ft_split(game->map_utils->tmp_map, '\n');
	free(game->map_utils->tmp_map);
}